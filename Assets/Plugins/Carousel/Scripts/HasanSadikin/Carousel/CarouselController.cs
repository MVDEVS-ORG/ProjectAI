    ...
    public class CarouselController<T> : MonoBehaviour
    {
        ...
        protected virtual void UpdateData()
        {
            for (int i = 0; i < _carouselItems.Count; i++)
            {
                ...
                bool isActive = _isInfinity ? i == GetCarouselIndex(_currentIndex + _data.Count * _indexRepeatOffset) : i == _currentIndex;
                ...
                MoveItemToPositionAtIndex(item, _isInfinity ? GetCarouselIndex(i - _currentIndex) - _data.Count * _indexRepeatOffset : i - _currentIndex);
            }
        }
        ...
        {
            var targetItem = GetCarouselItemAt(_currentIndex + _data.Count);
            int direction = _positioner.IsItemAfter(targetItem._rectTransform, clickedItem._rectTransform) ? -1 : 1;

            while (GetCarouselItemAt(_currentIndex + _data.Count) != clickedItem)
            {
